...
import com.github.krystiankowalik.jooqexample.db.TestDbDAO;
import com.github.krystiankowalik.jooqexample.domain.Tables;
...
import com.github.krystiankowalik.jooqexample.model.Post;
...
import static com.github.krystiankowalik.jooqexample.domain.Tables.POSTS;
import static com.github.krystiankowalik.jooqexample.domain.tables.Comments.COMMENTS;
...
public class BlogService {
    ...
    public Comment getCommentAndPost(Integer commentId) {
        Record record = dsl.select().from(COMMENTS).join(POSTS).on(COMMENTS.POST_ID.eq(POSTS.ID)).where(COMMENTS.ID.eq(commentId)).fetchOne();
        Comment commentEntity = getCommentEntity(record);
        commentEntity.setPost(
                getPostEntity(record)
        );
        return commentEntity;
    }

    public List<Comment> getAllComments(){
        return testDbDAO.getComments();
    }
